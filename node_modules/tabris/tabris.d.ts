// Type definitions for Tabris.js 2.0.0-rc2
...
interface FileSystemEvents<T> extends NativeObjectEvents<T> {
}
...
export class FileSystem extends NativeObject {
   ...
   * @param listeners A key-value map where the keys are event types and the values are the listeners to deregister from these events, e.g. `{tap: onTap, scroll: onScroll}`.
   */
  off(listeners: FileSystemEvents<this>): this;
   ...
   * @param listeners A key-value map where the keys are event types and the values are the listeners to register for these events, e.g. `{tap: onTap, scroll: onScroll}`.
   */
  on(listeners: FileSystemEvents<this>): this;
   ...
   * @param listeners A key-value map where the keys are event types and the values are the listeners to register for these events, e.g. `{tap: onTap, scroll: onScroll}`.
   */
  once(listeners: FileSystemEvents<this>): this;
   ...
   * @param properties 
   */
  set(properties: FileSystemProperties): this;

  /**
   * Writes the given contents to the given file. If the file exists, it is overwritten, otherwise it is
   * created. Returns a promise that resolves on success and rejects with an Error in case of a failure.
  ...
  /**
   * The path to a directory that the app may use to store cached files. The OS may delete files in this
 ...
 * singleton instance can be accessed via `ui.navigationBar` and is only available on Android.
 */
...
export class NavigationView extends Composite {
   ...
   * topmost page as the last element. Same as children(), but does not return children of other types
   * than `Page`.
   * @param selector A selector expression or a predicate function to filter the results.
   */
  pages(selector?: Selector): WidgetCollection;
...
interface _StatusBarProperties extends _WidgetProperties {
   ...
   * room for the content. The `float` option lets the content flow underneath the status bar.
   */
  displayMode: "default" | "float" | "hide";
